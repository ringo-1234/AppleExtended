...
object GeneratedResourcePack : IResourcePack,
    ...
    override fun resourceExists(location: ResourceLocation): Boolean =
        location.resourceDomain == DOMEIN && location.resourcePath in resourceGenerators
    ...
    override fun getInputStream(location: ResourceLocation): InputStream {
        ...
        var resource = resources[location.resourcePath]
        if (resource == null) {
            val generator = resourceGenerators[location.resourcePath]
                ?: throw FileNotFoundException(location.resourcePath)
            resource = generator()
            resources[location.resourcePath] = resource
        }

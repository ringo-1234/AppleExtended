...
public class ProtectionManager
{
	...
	{
		Object target = this.getTarget(player.world, x, y, z);
	...
	private boolean useKey(EntityPlayer player, NBTTagCompound keyNBT, Object target)
	{
		...
		else
		{
			...
			{
				Lockable lockable = (Lockable)target;
	...
	private String getObjectName(Object object)
	{
		...
		else if(object instanceof TileEntity)
		{
			...
			return String.format("TileEntity:%s", tileEntity.getPos().toString());
		}
	...
	private void lockObject(EntityPlayer player, String name, String id, int lockType)
	{
		...
		nbt.setBoolean(KEY_UNBREAKABLE, (lockType & 1) != 0);
		nbt.setBoolean(KEY_UNEDITABLE, (lockType & 2) != 0);
	...
	public void loadData(World world)
	{
		...
		{
			this.lockObjs = (ProtectionData)world.loadData(ProtectionData.class, DATA_NAME);

			if(this.lockObjs == null)
			{
				this.lockObjs = new ProtectionData(DATA_NAME);
				world.setData(DATA_NAME, this.lockObjs);
			}
		}
	}

...
public class GuiSelect extends GuiScreenCustom {
    ...
    public static class SlotElementItem<T> extends SlotElement {
        ...
        public static void func_148171_c(Minecraft par1, int par2, int par3, int par4, int par5, float par6) {
            ...
            par1.getTextureManager().bindTexture(Gui.STAT_ICONS);
            float f0 = 1.0F / 128.0F;
            int iconSize = 18;
            NGTTessellator tessellator = NGTTessellator.instance;
            tessellator.startDrawingQuads();
            NGTRenderHelper.addQuadGuiFaceWithUV(par2, par3, par2 + iconSize, par3 + iconSize, par6,
                    (float) par4 * f0, (float) par5 * f0, (float) (par4 + iconSize) * f0, (float) (par5 + iconSize) * f0);
            tessellator.draw();
        }

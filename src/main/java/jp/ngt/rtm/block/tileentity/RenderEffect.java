...
public final class RenderEffect extends TileEntitySpecialRenderer<TileEntityEffect>
{
	...
	public RenderEffect()
	{
		Thread thread = new Thread(){
			...
			public void run()
			{
				RenderEffect.this.model = ModelLoader.loadModel(new ResourceLocation("rtm", "models/explosion_n.mqo"), VecAccuracy.MEDIUM);
				RenderEffect.this.sphere = ModelLoader.loadModel(new ResourceLocation("rtm", "models/explosion_s.mqo"), VecAccuracy.MEDIUM);
				RenderEffect.this.ring = ModelLoader.loadModel(new ResourceLocation("rtm", "models/explosion_r.mqo"), VecAccuracy.MEDIUM);
	...
	private void render(TileEntityEffect entity, double x, double y, double z, float partialTick)
	{
		...
		{
			ItemStack item = NGTUtilClient.getMinecraft().player.getHeldItemMainhand();
			if(!item.isEmpty() && item.getItem().getUnlocalizedName().contains("effect"))
			{
				GL11.glPushMatrix();
				GL11.glEnable(GL12.GL_RESCALE_NORMAL);
				GL11.glTranslatef((float)x + 0.5F, (float)y + 0.5F, (float)z + 0.5F);

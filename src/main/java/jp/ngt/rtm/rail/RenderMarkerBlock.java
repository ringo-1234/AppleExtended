...
public final class RenderMarkerBlock extends TileEntitySpecialRenderer<TileEntityMarker>
{
	...
	private void renderGUI(TileEntityMarker marker)
	{
		if(marker.gui == null)
		{
			...
			marker.gui = new InternalGUI(startX, startY, buttonWidth + 0.2F, guiHeight).setColor(0x00FFFF);
			marker.buttons = new InternalButton[5];
			startX += 0.1F;
			startY += 0.1F;
			marker.buttons[0] = (new InternalButton(startX, startY, buttonWidth, buttonHeight)).setColor(buttonColor).setListner((button)->{
						marker.flipState(MarkerState.ANCHOR21);
						});
			startY += buttonHeight + 0.1F;
			marker.buttons[1] = (new InternalButton(startX, startY, buttonWidth, buttonHeight)).setColor(buttonColor).setListner((button)->{
						marker.flipState(MarkerState.LINE2);
					});
			startY += buttonHeight + 0.1F;
			marker.buttons[2] = (new InternalButton(startX, startY, buttonWidth, buttonHeight)).setColor(buttonColor).setListner((button)->{
						marker.flipState(MarkerState.LINE1);
					});
			startY += buttonHeight + 0.1F;
			marker.buttons[3] = (new InternalButton(startX, startY, buttonWidth, buttonHeight)).setColor(buttonColor).setListner((button)->{
						marker.flipState(MarkerState.GRID);
					});
			startY += buttonHeight + 0.1F;
			marker.buttons[4] = (new InternalButton(startX, startY, buttonWidth, buttonHeight)).setColor(buttonColor).setListner((button)->{
	...
	private void renderDistanceMark(TileEntityMarker marker)
	{
		...
		float dir = (float)BlockMarker.getMarkerDir(marker.getBlockType(), meta) * 45.0F;
		...
		for(int i = 0; i < this.displayStrings.length; ++i)
		{
			...
			for(int j = -1; j <= 1; ++j)
			{
				...
				GL11.glRotatef(-NGTUtilClient.getMinecraft().getRenderManager().playerViewY - dir, 0.0F, 1.0F, 0.0F);
	...
	private MarkerElement renderAnchorLine(TileEntityMarker marker, boolean isPickMode, MarkerElement hoveredElement)
	{
		...
		if(marker.getState(MarkerState.LINE2) && marker.isCoreMarker())//始点側のみ描画
		{
			GL11.glPushMatrix();
		...
		if(marker.getState(MarkerState.LINE1))
		{
			...
			GL11.glPushMatrix();
			if(marker.isCoreMarker() && marker.getRailMaps() != null && marker.getRailMaps().length == 1)//分岐なし
			{
		...
		GL11.glEnable(GL11.GL_TEXTURE_2D);
		if(marker.getState(MarkerState.LINE1) && !isPickMode)//数値描画
		{
			FontRenderer fontRenderer = NGTUtilClient.getMinecraft().getRenderManager().getFontRenderer();
			float scale = 0.04F;
			GL11.glPushMatrix();
	...
	private boolean changeAnchor(TileEntityMarker marker)//Minecraft.1518-this.objectMouseOver.hitVec
	{
		if(marker.editMode == 0 || marker.getCoreMarker() == null){return false;}

		if(marker.editMode > 0 && Mouse.isButtonDown(1))//アンカー固定
		{
			marker.editMode = 0;
		...
		if(marker.getState(MarkerState.LINE1))
		{
			...
			if(dx != 0.0D && dz != 0.0D)
			{
				...
				{
					float pitch = MathHelper.wrapDegrees(yaw - rp.anchorYaw);
					if(neighborRP != null)
					{
	...
	{
		int markerDir = tileEntity.getMarkerRP().direction;

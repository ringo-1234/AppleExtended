...
public class BlockLargeRailBase extends BlockContainerCustomWithMeta
{
    ...
    {
        return BlockFaceShape.UNDEFINED;//柵とくっつかないように
    }
	...
	protected AxisAlignedBB getAABBWithState(IBlockAccess world, BlockPos pos)
    {
		...
		if(tile == null){return FULL_BLOCK_AABB;}//破壊時ぬるぽ防止

		int x = pos.getX();
    	int y = pos.getY();
    	int z = pos.getZ();
		float[] fa = ((TileEntityLargeRailBase)tile).getBlockHeights(x, y, z, THICKNESS, false);//trueだとデフォルト高で返される
		float height2 = 0.0F;
		for(int i = 0; i < 4; ++i)
		{
			height2 += fa[i];
	...
	public void onEntityCollidedWithBlock(World world, BlockPos pos, IBlockState state, Entity entity)
    {
		if(world.isRemote)
		{
			...
			if(rail == null || rail.getResourceState() == null){return;}//一時しのぎ

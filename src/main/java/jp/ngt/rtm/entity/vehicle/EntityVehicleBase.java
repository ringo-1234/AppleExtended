...
public abstract class EntityVehicleBase<T extends ModelSetVehicleBase> extends EntityCustom implements IResourceSelector
{
    ...
    /**spawnさせるときClient側でインスタンス生成しないこと*/
	public EntityVehicleBase(World world)
	{
		...
		if(!NGTUtil.isServer())
		{
			setRenderDistanceWeight(16.0D);//どれ位がいいか未検証
		}
		...
		for(TrainStateType type : TrainStateType.values())//DataMap初期化(キーをマッピングしておくため)
		{
    ...
    {
		//nullじゃないとバニラパケットで移動が同期される
		//NetHandlerPlayerServer.processVehicleMove()で"moved wrongly!"と出る
	...
	public void fixRiderPosOnDismount(Entity passenger, Entity ridingEntity)
	{
		...
		catch(NullPointerException e)//マルチでぬるぽになる応急処置
		{
			e.printStackTrace();
		}
		...
		//size[1] += this.getVehicleYOffset();
		//size[4] += this.getVehicleYOffset();
		Vec3 vec = PooledVec3.create(ridingEntity.posX - this.posX, ridingEntity.posY - this.posY, ridingEntity.posZ - this.posZ);
		//yaw=0に補正して乗り物のAABBとの処理を実施
		vec = vec.rotateAroundY(-this.rotationYaw);
		float yawDif = NGTMath.wrapAngle(-passenger.rotationYaw - this.rotationYaw);//PlayerのYawは逆周り
		//目線方向によって乗り物の左右どちらかにAABBサイズ分ずらす
		...
		if(vec.length() <= 16.0D)
		{
			...
			if(newY - y >= maxY)//降りられない場合は左右反対側へ
			{
				vec = PooledVec3.create(-fixX, vec.getY(), vec.getZ());
				vec = vec.rotateAroundY(this.rotationYaw);
	...
	/**Server Only*/
	private void setupFloors()//getModelSetでループしないように
	{

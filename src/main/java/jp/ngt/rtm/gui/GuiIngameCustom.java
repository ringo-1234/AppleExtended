...
public class GuiIngameCustom extends GuiScreen
{
	...
	public GuiIngameCustom(Minecraft par1)
    {
		super();
        this.mc = par1;
        this.zLevel = -100.0F;//ホットバーを隠さないため
    }

	//HELMET→HOTBARの順で描画される
	...
	private void renderTimeTable(EntityTrainBase train)
	{
		...
		for(int i = 0; i < tt.length; ++i)
		{
			...
			renderer.drawString(map.get(i), x, y + fontH * i, color);//駅
			renderer.drawString(entry.data[1], x + w1, y + fontH * i, color);//着時刻
			renderer.drawString(entry.data[2], x + w1 + w2, y + fontH * i, color);//発時刻
		}
	}
	...
	private void drawMeterAndLever(EntityTrainBase train)
	{
		...
		float max = train.getResourceState().getResourceSet().getConfig().maxSpeed[4];
		float r0 = 270.0F * train.getSpeed() / max;
		this.drawMeter(halfW - 136, this.height - 29, 32, 64, 48, r0, 512);

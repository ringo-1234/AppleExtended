...
public class GuiVehicleControlPanel extends InventoryEffectRenderer
{
	...
	//private GuiTextField vehicleNameField;
	...
	private void setCurrentTab(TabTrainControlPanel tab)
	{
		...
		else if(tab == TabTrainControlPanel.TAB_Setting)
		{
			...
			/*this.vehicleNameField = new GuiTextField(0, this.fontRendererObj,
					this.guiLeft + 82, this.guiTop + 6, 89, this.fontRendererObj.FONT_HEIGHT);
			this.vehicleNameField.setMaxStringLength(15);
			this.vehicleNameField.setEnableBackgroundDrawing(false);
			this.vehicleNameField.setTextColor(16777215);*/
		}
		else if(tab == TabTrainControlPanel.TAB_Function)
		{
			...
			for(int i = 0; i < buttons.length; ++i)
			{
				...
				GuiButtonCustom button = new GuiButtonCustom(CUSTOM_BUTTOM_ID + i, x, y, 54, 20, buttons[i][value], this);
		...
		boolean r = (state & 1) == 1;
		boolean l = (state & 2) == 2;
		boolean dir = this.vehicle.getVehicleState(TrainStateType.Direction) == 0;
	...
	{
		if(this.selectedTabIndex == TabTrainControlPanel.TAB_Setting.getTabIndex())
		{
			if(this.vehicleNameField.textboxKeyTyped(typedChar, keyCode))
			{
	...
	protected void drawGuiContainerBackgroundLayer(float par1, int par2, int par3)
	{
		...
		//this.vehicleNameField.drawTextBox();
	}
	...
	protected void actionPerformed(GuiButton button)
	{
		...
		if((button.id >= 110 && button.id <= 115) || (button.id >= 124 && button.id <= 129) || (button.id >= 140 && button.id <= 142))
		{
			...
			else if(button.id == 129)
			{
				...
				{
					RTMCore.proxy.playSound(this.vehicle, sa0[index][1], 1.0F, 1.0F);
				}
				return;
			}

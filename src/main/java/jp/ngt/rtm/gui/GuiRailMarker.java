...
public class GuiRailMarker extends GuiScreenCustom
{
	...
	public void initGui()
    {
		...
		this.fieldArgs       = this.setTextField(this.width - 140, 170, 130, 20, args);
	...
	protected void actionPerformed(GuiButton button)
    {
		...
		else if(button.id == 100)
        {
			...
			/*List<String> list = ModelPackManager.INSTANCE.getLoadedScripts();
			this.mc.displayGuiScreen(new GuiSelectText(this, list, (s)->{
				this.currentRP.scriptName = s;
			}));*/
			//レール形状スクリプト選択、未実装
        }
	...
	protected void keyTyped(char par1, int par2)
    {
		...
		if(this.currentTextField != null)
		{
			if((par2 >= Keyboard.KEY_1 && par2 <= Keyboard.KEY_0) ||
					(par2 >= Keyboard.KEY_UP && par2 <= Keyboard.KEY_RIGHT) ||
					par2 == Keyboard.KEY_MINUS || par2 == Keyboard.KEY_BACK || par2 == Keyboard.KEY_DELETE || par2 == Keyboard.KEY_PERIOD)
			{

...
public class WirePartsRenderer extends TileEntityPartsRenderer<ModelSetWire>
{
	...
	public void renderWire(TileEntityElectricalWiring tileEntity, Connection connection, Vec3 target, float par8, RenderPass pass)
	{
		...
		this.renderWireStatic(tileEntity, connection, target, par8, pass.id);//スクリプトからの呼び出し考慮しpassはint渡し
	...
	protected void renderWireStatic(TileEntityElectricalWiring tileEntity, Connection connection, Vec3 target, float par8, int pass)
	{
		if(this.useScript)
		{
			ScriptUtil.doScriptFunction(this.getScript(), "renderWireStatic", tileEntity, connection, target, par8, pass);
		}
		else
		{
			;
	...
	protected void renderWireDynamic(TileEntityElectricalWiring tileEntity, Connection connection, Vec3 target, float par8, int pass)
	{
		if(this.useScript)
		{
			ScriptUtil.doScriptFunction(this.getScript(), "renderWireDynamic", tileEntity, connection, target, par8, pass);
		}
		else if(pass == RenderPass.NORMAL.id)//スクリプトなしモデルでの発光防止
		{
			...
			if(canUseColor)
	    	{
				GLHelper.setColor(connection.getResourceState().color, 0xFF);
	    	}
	...
	public void renderWireStraight(TileEntityElectricalWiring tileEntity, Connection connection, Vec3 target, float par8, int pass, @Nullable Parts parts)
	{
		...
		int split = NGTMath.floor(length / cfg.sectionLength);
	...
	/*@Deprecated
	protected void renderWireDeflection2(TileEntityElectricalWiring tileEntity, Connection connection, Vec3 target, float par8)
	{
		...
		while(x < lx)
		{
			...
			float dx = NGTMath.getCos((float)slopeRad) * cfg.sectionLength;
	...
	public void renderWireDeflection(TileEntityElectricalWiring tileEntity, Connection connection, Vec3 target, float par8, int pass, @Nullable Parts parts)
	{
		...
		while(x < lx)
		{
			...
			float dx = NGTMath.getCos((float)slopeRad) * cfg.sectionLength * 0.99F;

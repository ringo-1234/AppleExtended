...
{
	NOTCH(
			(s)->{return s.equals("notch");},
			(obj)->{return obj instanceof EntityTrainBase;},
	...
	DATA_MAP(
			(s)->{return s.startsWith("dm:");},
			(obj)->{return obj instanceof IResourceSelector;},
			(target, player, order, value)->{
	...
	VEHICLE_STATE(
			(s)->{return s.startsWith("state:");},
			(obj)->{return obj instanceof EntityVehicleBase;},
			(target, player, order, value)->{
	...
	V_MOV_DIST(
			(s)->{return s.equals("move");},
			(obj)->{return obj instanceof EntityVehicle;},
			(target, player, order, value)->{
	...
	V_ADD_YAW(
			(s)->{return s.equals("addYaw");},
			(obj)->{return (obj instanceof EntityVehicle) || (obj instanceof EntityArtillery);},
			(target, player, order, value)->{
	...
	FIRE(
			(s)->{return s.equals("fire");},
			(obj)->{return obj instanceof EntityArtillery;},
			(target, player, order, value)->{
				EntityArtillery firearm =((EntityArtillery)target);
	...
	NO_FUNC((s)->{return false;}, (obj)->{return false;}, (target, player, order, value)->{return false;}, "");

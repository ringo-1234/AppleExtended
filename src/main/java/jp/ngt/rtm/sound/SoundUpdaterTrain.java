...
public class SoundUpdaterTrain extends SoundUpdaterVehicle<EntityTrainBase>
{
	...
	public void update()
	{
		...
		if(Math.abs(train.getSpeed()) > 0.0F)
		{
			switch(signal)
			{
				case 1:
					if(this.currentSignal != 1)
					{
						...
						this.atsSound[0] = MovingSoundMaker.create(bogie, modelSet.getConfig().sound_ATSChime, true);
						this.soundHandler.playSound(this.atsSound[0]);
						this.atsSound[1] = MovingSoundMaker.create(bogie, modelSet.getConfig().sound_ATSBell, true);

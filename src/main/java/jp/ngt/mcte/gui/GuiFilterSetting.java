...
public class GuiFilterSetting extends GuiScreenCustom {
    ...
    private void initFiltersButton() {
        ...
        for (EditFilterBase filter : this.filters) {
            this.filterButtons.add(new GuiButtonCustom(++buttonId, 10, 0, FILTER_BUTTON_W, 20, filter.getFilterName(), this));
            this.filterSettingButtons.add(new GuiButtonCustom(++buttonId, 10 + FILTER_BUTTON_W, 0, SETTING_BUTTON_W, 20, "⚙", this));
        }
    ...
    public void drawScreen(int par1, int par2, float par3) {
        ...
        NGTRenderHelper.addQuadGuiFace(SEPARATION_POS, 0.0F, this.width, this.height, this.zLevel);
        int y = (this.selectedFilterId - this.currentScrollL) * (20 + BUTTON_GAP_Y) + BUTTON_GAP_Y;
        NGTRenderHelper.addQuadGuiFace(0.0F, y - (BUTTON_GAP_Y / 2), SEPARATION_POS, y + 20 + BUTTON_GAP_Y, this.zLevel);
        tessellator.draw();
    ...
    private void updateElementsPos() {
        for (int i = 0; i < this.filterButtons.size(); ++i) {
            ...
            this.filterButtons.get(i).y = yPos;
            this.filterSettingButtons.get(i).y = yPos;
        }

...
public class WorldData
{
	...
	public Map<Integer, Biome> biomeMap = new TreeMap<Integer, Biome>();//getが早い
	...
	public WorldData(String par1)
	{
		...
		try{this.loadWorldData(file);}catch(IOException e){}
	}
		...
		for(String s : sa0)
		{
			...
			if(sa1.length == 2)
			{
				try
				{
					...
					Biome biome = getBiomeFromName(sa1[1]);
	...
	public static String getStringFromBiomeColorMap(Map<Integer, Biome> par1)
	{
		...
		Set<Entry<Integer, Biome>> set = par1.entrySet();
		for(Entry<Integer, Biome> entry : set)
		{
			sb.append(String.format("0x%06x", entry.getKey())).append(":").append(entry.getValue().getBiomeName()).append(",");
		}
		return sb.toString();
	}
	...
	public static Biome getBiomeFromName(String par1)
	{
		for(ResourceLocation res : Biome.REGISTRY.getKeys())
		{
			...
			if(biome != null && biome.getBiomeName().equals(par1))
			{
				return biome;
			}
		}
	...
	{
		if(this.terrainImagePath != null){this.cfg.setProperty("Image", "terrain", this.terrainImagePath);}
		if(this.biomesImagePath != null){this.cfg.setProperty("Image", "biomes", this.biomesImagePath);}
		...
		this.cfg.setProperty("Block", "baseBlock", Block.REGISTRY.getNameForObject(this.baseBlock).toString());
